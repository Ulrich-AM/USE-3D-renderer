    ...
    public static class Simulator
	{
		...
		static void ProcessBuiltinChip(SimChip chip)
		{
			switch (chip.ChipType)
			{
				...
				case ChipType.DisplayRGB8x8:
				{
					const uint addressSpace = 64;
					uint addressPin = chip.InputPins[0].State.GetShortValues();
					...
					if (isRisingEdge)
					{
						...
						{
							uint data = chip.InputPins[1].State.GetShortValues();
							chip.InternalState[addressPin + addressSpace] = data;
						}
					...
					// Output current pixel colour
					uint colData = chip.InternalState[addressPin];
					chip.OutputPins[0].State.SetShort((ushort)colData);


				break;
				}
				case ChipType.CustomDisplayRGB:
				{
					uint width = chip.InternalState[^3];
					uint height = chip.InternalState[^2];
					uint addressSpace = width * height;
					...
					// Output current pixel colour
					if (addressPin < addressSpace)
					{
						uint colData = chip.InternalState[addressPin];
						chip.OutputPins[0].State.SetShort(colData);
					}
				...
				case ChipType.DisplayRGB64x64_XY:
				{
					const uint addressSpace = 4096;
					uint x = chip.InputPins[0].State.GetShortValues();
					uint y = chip.InputPins[1].State.GetShortValues();
					uint address = y * 64 + x;

					bool writePin = chip.InputPins[6].State.SmallHigh();
					bool clockPin = chip.InputPins[8].State.SmallHigh();

					bool isRisingEdge = clockPin && chip.InternalState[^1] == 0;
					chip.InternalState[^1] = clockPin ? 1u : 0;

					if (isRisingEdge && address < addressSpace)
					{
						// Clear back buffer
						if (chip.InputPins[5].State.SmallHigh())
						{
							for (int i = 0; i < addressSpace; i++)
							{
								chip.InternalState[i + addressSpace] = 0;
						...
						{
							uint data = (chip.InputPins[2].State.GetShortValues() | (chip.InputPins[3].State.GetShortValues() << 4) | (chip.InputPins[4].State.GetShortValues() << 8));
							chip.InternalState[address + addressSpace] = data;
						}

						// Copy back-buffer to display buffer
						if (chip.InputPins[7].State.SmallHigh())
						{
							for (int i = 0; i < addressSpace; i++)
							{
								chip.InternalState[i] = chip.InternalState[i + addressSpace];
					...
					{
						uint colData = chip.InternalState[address];
						chip.OutputPins[0].State.SetShort((ushort)(colData & 0b1111));//red
						chip.OutputPins[1].State.SetShort((ushort)((colData >> 4) & 0b1111));//green
						chip.OutputPins[2].State.SetShort((ushort)((colData >> 8) & 0b1111));//blue
					}
				...
				case ChipType.DisplayRGB256x256_XY:
				{
					const uint addressSpace = 65536;
					uint x = chip.InputPins[0].State.GetShortValues();
					uint y = chip.InputPins[1].State.GetShortValues();
					uint address = y * 256 + x;
				...
				case ChipType.DisplayRGB64x64_XY_8bit:
				{
					const uint addressSpace = 4096;
					uint x = chip.InputPins[0].State.GetShortValues();
					uint y = chip.InputPins[1].State.GetShortValues();
					...
					if (isRisingEdge && address < addressSpace)
					{
						...
						{
							uint data = (chip.InputPins[2].State.GetShortValues() | (chip.InputPins[3].State.GetShortValues() << 8) | (chip.InputPins[4].State.GetShortValues() << 16));
							chip.InternalState[address + addressSpace] = data;
						}
					...
					{
						uint colData = chip.InternalState[address];
						chip.OutputPins[0].State.SetShort((ushort)(colData & 0xFF));//red
						chip.OutputPins[1].State.SetShort((ushort)((colData >> 8) & 0xFF));//green
						chip.OutputPins[2].State.SetShort((ushort)((colData >> 16) & 0xFF));//blue
					}

					break;
				}
				case ChipType.DisplayRGB256x256_XY_8bit:
				{
					const uint addressSpace = 65536;
					uint x = chip.InputPins[0].State.GetShortValues();
					uint y = chip.InputPins[1].State.GetShortValues();
				...
				case ChipType.BankedRom:
				{
					const uint mask = 0x00ff;
					uint selector = chip.InputPins[0].State.GetShortValues() & mask;
					uint address = chip.InputPins[1].State.GetShortValues() & mask;
					uint index = (selector << 8) | address;
					uint data = chip.InternalState[index];
				...
				case ChipType.CustomRom:
				{
					uint address = chip.InputPins[0].State.GetValue();
					if (address < chip.InternalState.Length)
					{
						uint data = chip.InternalState[address];
